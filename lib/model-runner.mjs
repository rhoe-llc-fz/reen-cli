...
export class ModelRunner {
  ...
  buildPrompt(model, history) {
    const sys = `You are ${model} in a multi-AI conference chat. Other participants: claude, codex, gemini, and human users. Respond naturally and concisely to the conversation. Do NOT talk about code, CLI tools, or the conference system itself — just answer the question asked. Reply in the same language as the user. When files are shared, analyze their content and respond about them. IMPORTANT: When addressing other participants, ALWAYS use @mentions: @claude, @codex, @gemini, or @all. Example: "@codex what do you think?" This is required for message routing.`;
