...
export class ModelRunner {
  ...
  buildPrompt(model, history) {
    const others = ['claude', 'codex', 'gemini'].filter(m => m !== model).join(', ');
    // Первое сообщение конференции = пользовательская инструкция
    const userPrompt = history.length > 0 ? history[0].content : '';

    const sys = `You are ${model}, a participant in a multi-model AI conference. Other participants: ${others}.
