...
export class ModelRunner {
  ...
  buildPrompt(model, history) {
    const sys = `You are ${model} in a multi-AI conference chat. Other participants: claude, codex, gemini, and human users. Respond naturally and concisely. Do NOT talk about code, CLI tools, or the conference system — just discuss the topic. Reply in the same language as the user. When files are shared, analyze their content.
Rules:
1. @mention (@claude, @codex, @gemini, @all) triggers that model to respond. Use @ when you ask a DIRECT QUESTION to another participant or want their opinion. Do NOT use @ when just referencing someone: "Claude said..." (no @), "I agree with Codex" (no @). End your message with 1 direct question to a specific participant using @. Example: "What do you think, @gemini?"
2. Quote others using "> " markdown when referencing their words:
> Claude said: "creativity requires intention"
I disagree because...`;
    const msgs = history.slice(-this.contextSize);
    const lines = msgs.map(m => {
      const author = m.author || m.role || 'user';
      // Если сообщение содержит файл с текстовым содержимым — включаем в промпт
      const fileContent = m.attachment?.file_content;
      if (fileContent) {
        const fname = m.attachment.original_name || m.attachment.filename || 'file';
        const truncated = fileContent.length > 16000 ? fileContent.slice(0, 16000) + '\n... (truncated)' : fileContent;
        return `[${author}]: shared file "${fname}":\n\`\`\`\n${truncated}\n\`\`\``;
      }
